...
| 디스크 여유 공간 | **5GB 권장** (Python·가상환경·라이브러리는 수백 MB 수준, 나머지는 VS Code·Docker(09장)·프로젝트 여유분) |
...
지금까지 우리는 응답을 **`dict`**로 만들어 돌려줬습니다. 이 방식은 짧은 예제엔 편하지만, 응답의 형태가 코드 어디에서 결정되는지 한눈에 보기 어렵고, 자동 문서의 응답 스키마도 `additionalProperties`(임의의 키-값) 형태로 뭉뚱그려져 어떤 키가 오는지 드러나지 않습니다.
...
단, 위 "조용한 절단"은 옛 동작이고 **`bcrypt` 4.x/5.x는 72바이트 초과 시 절단하지 않고 `ValueError`를 던진다** — 그래서 `hash_password`의 사전 길이검사가 이 에러를 사용자 친화적 메시지로 바꿔준다(§8.17.3 참고).
...
3. **`.env.example`** 이라는 예시 파일은 커밋한다 — 안에는 진짜 키가 아닌 32바이트 이상의 더미 값(`SECRET_KEY=please-change-this-to-32-bytes-or-longer-random-string`)만 넣는다.
...
`expires_minutes`가 `None`이면 설정값(`settings`의 액세스 토큰 만료시간)을 사용합니다. 자세한 구현은 8.10절(예제 코드)에서 다룹니다.
...
class Settings(BaseSettings):
    ...
    def _check_secret_key(self) -> "Settings":
        """더미 secret_key 가 운영에서 그대로 쓰이는 사고를 방지한다.

        APP_ENV=production 환경 변수가 설정된 상태에서 더미 prefix 가 감지되면
        즉시 부팅을 중단해 운영 사고를 막는다. 그 외 환경에서는 경고만 표시.
        """
...
> **더미 키 부팅 차단의 의도** — 위 `_check_secret_key` 검증은 `Settings()` 가 import 시 만들어질 때 자동 실행됩니다. 운영 배포에서 누군가 `.env` 주입을 잊었더라도 첫 부팅에서 `RuntimeError` 로 즉시 멈추므로, "기본 더미 키가 운영에 들어가 토큰이 위조되는" 종류의 사고를 막습니다. 개발 환경에서는 경고만 표시되어 학습 흐름을 끊지 않습니다.
...
def decode_access_token(token: str) -> TokenPayload:
    ...
    `options={"require": [...]}` 로 필수 클레임을 강제한다 — exp/sub/iat 가
    없는 토큰은 통과시키지 않는다(만료 없는 토큰을 막기 위함).
    """
    raw = jwt.decode(
        token,
        settings.secret_key,
...
def create_app() -> FastAPI:
    ...
    # `allow_origins=["*"]`와 `allow_credentials=True`는 CORS 규약상 함께 쓸 수 없다.
    # 이 예제는 헤더 기반 JWT라 credentials가 불필요하므로 False로 둔다.
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
        allow_methods=["*"],
...
async def client():
    engine = create_async_engine(
        ...
        # 여러 세션이 같은 인메모리 DB를 보려면 StaticPool이 필요(같은 연결 재사용).
...
> **버전 (2026-04 기준)**: 2.x (1.x도 호환, 최소 1.20+ 권장)
> **설치**: `uv add --dev mypy`
...
2. Bcrypt 4.x/5.x와 호환 경고가 자주 발생합니다.
...
# v7 (시간순 정렬 가능). 단, 표준 uuid 의 uuid7() 은 Python 3.14+ 에서만 제공됩니다.
# 이 가이드 기준인 3.13 이하에는 uuid7 이 없어 아래 줄은 AttributeError 가 납니다.
# 3.13 이하에서는 uuid6 / uuid-utils 패키지나 ULID 를 사용하세요.
uid7 = uuid.uuid7()   # Python 3.14+ 에서만. 3.13 이하는 uuid6/uuid-utils 패키지 사용
```
...
> **v7은 Python 3.14+ 표준 `uuid`에서만 제공되며, 3.13 이하에는 없습니다.** 이 가이드는 Python 3.13 기준이므로 `uuid.uuid7()` 을 그대로 부르면 `AttributeError` 가 납니다. 3.13 이하에서 시간순 정렬 UUID 가 필요하면 외부 패키지(`uuid6` 또는 `uuid-utils`)나 ULID 를 사용하세요.

### 함정

- UUID v4는 **시간 정렬이 안 됩니다**. DB 인덱스에 부담을 줄 수 있습니다 → v7 또는 ULID.
- `uuid.uuid7()`은 **Python 3.14에서 추가**됐습니다. 즉 **3.13 이하 표준 `uuid`에는 `uuid7`이 없습니다**(`hasattr(uuid, "uuid7")` 가 `False`). 3.14 이상 새 프로젝트는 표준 v7을 검토하고, 3.13 이하는 ULID(ulid-py) 또는 외부 라이브러리(`uuid6`/`uuid-utils`)를 사용하세요.
...
- `--proxy-headers --forwarded-allow-ips`: nginx/리버스 프록시 뒤에서 `X-Forwarded-Proto`/`X-Forwarded-For` 신뢰. 없으면 scheme/IP 가 잘못 잡힙니다. 기본값 `127.0.0.1`(같은 호스트의 프록시)이 안전하고, 여러 프록시/오케스트레이터(LB·k8s 등) 뒤에서는 신뢰 대역으로 넓힙니다.
...
> **주의**: 옛 `gunicorn -k uvicorn.workers.UvicornWorker` 는 Uvicorn 0.30 부터 deprecated 되어 별도 패키지(`uvicorn-worker`)로 분리되었습니다. 위처럼 `uvicorn_worker.UvicornWorker` 를 쓰세요.
...
- 운영: **Uvicorn 자체 멀티워커**(`--workers N`)가 표준. Gunicorn 의 운영 기능(graceful reload 등)이 필요하면 별도 패키지 `uvicorn-worker` 와 함께 쓴다. `uvicorn[standard]`만 깔아도 uvloop/httptools 같은 성능 부품이 자동 포함.
