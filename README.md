...
| Uvicorn | 0.30.x 이상 | 우리가 만든 FastAPI 앱을 실행하는 ASGI 서버. 운영에선 자체 멀티워커(`--workers N`)로 여러 프로세스를 띄워 부하를 분산 |
| Gunicorn | 23.x (선택) | graceful reload 등 운영 기능이 필요할 때 `uvicorn-worker`와 함께 쓰는 프로세스 매니저 (기본 권장은 Uvicorn 자체 멀티워커) |
